...
class Isu_alamsekitar_model extends CI_Model
{

    public function senarai_laporan($pelapor, $tahun)
    {
        $namaTable = 'alamsekitar_'.$pelapor.'_'.$tahun;
        if($this->db->table_exists('alamsekitar_'.$pelapor.'_'.$tahun) != FALSE){
            $this->db->select('*');
            ...
            $query = $this->db->get('alamsekitar_'.$pelapor.'_'.$tahun);
            return $query->result();
        }
    }
    ...
    public function tambah()
    {
        ...
        $isu_alamsekitar = $this->input->post('input_tajuk_isu');
        if(!empty($lain) && $isu_alamsekitar == 'Lain-lain'){
            $isu_alamsekitar = $lain;
        }
        $data = array(
            ...
            'isu_alamsekitar' => $isu_alamsekitar,
            'cadangan_intervensi' => $this->input->post('input_cadangan_intervensi'),
            'tapisan' => 'Terima'
        );
        $this->db->insert('alamsekitar_'.$pelapor.'_'.date("Y"), $data);
    }
    ...
    {   
        ...
        if($this->db->table_exists($namaTable) == FALSE){
            ...
            $this->dbforge->create_table('alamsekitar_'.$pelapor.'_'.$tahun, TRUE);
        }
