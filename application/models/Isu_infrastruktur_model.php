...
class Isu_infrastruktur_model extends CI_Model
{

    public function senarai_laporan($pelapor, $tahun)
    {
        $namaTable = 'infrastruktur_'.$pelapor.'_'.$tahun;
        if($this->db->table_exists('infrastruktur_'.$pelapor.'_'.$tahun) != FALSE){
            $this->db->select('*');
            ...
            $query = $this->db->get('infrastruktur_'.$pelapor.'_'.$tahun);
            return $query->result();
        }
    }
    ...
    public function tambah()
    {
        ...
        $isu_infrastruktur = $this->input->post('input_tajuk_isu');
        if(!empty($lain) && $isu_infrastruktur == 'Lain-lain'){
            $isu_infrastruktur = $lain;
        }
        $data = array(
            ...
            'isu_infrastruktur' => $isu_infrastruktur,
            'kategori' => $this->input->post('inputKategori'),
            'tapisan' => 'Terima'
        );
        $this->db->insert('infrastruktur_'.$pelapor.'_'.date("Y"), $data);
    }
    ...
    {   
        ...
        if($this->db->table_exists($namaTable) == FALSE){
            ...
            $this->dbforge->create_table('infrastruktur_'.$pelapor.'_'.$tahun, TRUE);
        }
        ...
        if($this->db->table_exists($namaTable) == TRUE){
            ...
            if ($this->db->field_exists('kategori', $namaTable) == FALSE)
            {   
                $fields = array(
                    'kategori' => array(
                        'type' => 'TEXT',
