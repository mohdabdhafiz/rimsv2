...
class Isu_politik_model extends CI_Model
{

    public function senarai_laporan($pelapor, $tahun)
    {
        $namaTable = 'politik_'.$pelapor.'_'.$tahun;
        if($this->db->table_exists('politik_'.$pelapor.'_'.$tahun) != FALSE){
            $this->db->select('*');
            ...
            $query = $this->db->get('politik_'.$pelapor.'_'.$tahun);
            return $query->result();
        }
    }
    ...
    public function tambah()
    {
        ...
        $isu_politik = $this->input->post('input_tajuk_isu');
        if(!empty($lain) && $isu_politik == 'Lain-lain'){
            $isu_politik = $lain;
        }
        $data = array(
            ...
            'isu_politik' => $isu_politik,
            'cadangan_intervensi' => $this->input->post('input_cadangan_intervensi'),
            'tapisan' => 'Terima',
        );
        $this->db->insert('politik_'.$pelapor.'_'.$tahun, $data);
    }
    ...
    {   
        ...
        if($this->db->table_exists($namaTable) == FALSE){
            ...
            $this->dbforge->create_table('politik_'.$pelapor.'_'.$tahun, TRUE);
        }
