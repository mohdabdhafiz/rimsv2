...
<div class="p-3 border rounded mb-3">
    <h1><?= strtoupper($dun->dun_nama) ?></h1>
    <div class="row g-3 mt-3">
        <div class="col-12 col-lg-4">
            <?php echo anchor(base_url(), 'LAMAN UTAMA', "class='btn btn-sm btn-outline-primary w-100'"); ?>
        </div>
        <div class="col-12 col-lg-4">
            <a href="<?= site_url('perumus/dashboardDun/'.$dun->dun_bil) ?>" class="btn btn-sm btn-outline-primary w-100">DASHBOARD</a>
        </div>
        <div class="col-12 col-lg-4">
            <?php 
            $negeri = $data_negeri->negeri_nama($dun->dun_negeri);
            echo anchor('perumus/maklumat_negeri/'.$negeri->nt_bil, 'SENARAI DUN '.strtoupper($negeri->nt_nama), "class='btn btn-sm btn-outline-primary w-100'"); ?>
        </div>
    </div>
</div>
<?php 
...
foreach($senarai_pilihanraya as $pru): 
    $semakan_jangkaan = $data_calon_dun->semak_jangkaan($dun->dun_bil, $pru->pilihanraya_bil);
    $senaraiPelapor = $dataPelapor->senaraiPenggunaIkutDun($dun->dun_bil);
...
<div class="p-3 border rounded mb-3">
    ...
    <div class="p-3 border rounded my-3">
        <div class="float-end">
            <?= form_open('japen/set_semula_dun') ?>
        ...
        <div class="table-responsive mt-3">
            <table class="table table-sm table-bordered table-hover">
                ...
                <?php $bilangan = 1;
                    $tmp = $data_calon_dun->senarai_calon_tanpa_grading($dun->dun_bil, $pru->pilihanraya_bil);
                    $senarai_calon = $data_calon_dun->senarai_calon_grading_tarikh($dun->dun_bil, $pru->pilihanraya_bil, $tarikh);
                if(empty($senarai_calon) || count($senarai_calon) != count($tmp)){
                    $senarai_calon = $data_calon_dun->senaraiCalonDenganGrading($dun->dun_bil, $pru->pilihanraya_bil);
                }
                ...
                <tr>
                    <td><?= $bilangan++ ?></td>
                    <td><?php echo $calon->ahli_nama; ?></td>
                    <td><?php echo $parti->parti_nama; ?> - <?php echo $parti->parti_singkatan; ?></td>
                    <td>
                        <?php if($calon->pencalonan_jangkaan_japen != 'MENANG'){ ?>
                        <?php echo form_open('japen/pilihan_calon_dun'); ?>
                        <input type="hidden" name="input_calon_bil" value="<?= $calon->pencalonan_bil ?>">
                        <input type="hidden" name="input_dun_bil" value="<?= $dun->dun_bil ?>">
    ...
    <div class="p-3 border rounded mb-3">
        ...
        <div class="row g-3">
            <?php $senarai_harian = $data_harian->senarai_harian($dun->dun_bil, $pru->pilihanraya_bil); 
            foreach($senarai_harian as $arkib):
            ?>
            <div class="col-12 col-lg-2">
                <p class="small text-muted p-3 border text-center bg-light"><?php echo anchor('perumus/maklumat_harian_dun/'.$arkib->harian_bil, date_format(date_create($arkib->harian_tarikh), "d.m.Y")); ?></p>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <?php 
    $hari_ini = date('Y-m-d');
                $bilangan_pengundi = $data_dm->jumlah_pengundi_dun($dun->dun_bil)->jumlah;
                $dun = $data_dun->dun_bil($dun->dun_bil);
                $pengundi = $data_pengundi->pengundi($dun->dun_bil, $pru->pilihanraya_bil);
                if($pengundi){
                    $bilangan_pengundi = $pengundi->pengundi_jumlah;
                }
        ?>


        <?php 
        $harian = $data_harian->sedia_ada($dun->dun_bil, $pru->pilihanraya_bil);
        $tarikh = $harian->harian_tarikh;
        ...
        $harian_pengguna_waktu = $harian->harian_waktu;
        ?>
        <div class="table-responsive">
            <table class="table table-border table-hover table-bordered">
                ...
                <tr>
                    ...
                    <td><?= number_format($data_dm->jumlah_pengundi_dun($dun->dun_bil)->jumlah, 0, "", ","); ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><?= date_format(date_create($harian_pengguna_waktu), "d.m.Y H:i:s") ?></td>
                </tr>
            </table>
        </div>
        <?php
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    <?php 
                    $senarai_dm = $data_dm->dun($dun->dun_bil);
                    $jumlah_pengundi = 0;
                    foreach($senarai_dm as $dm): 
                        $seratus = 0;
                    $harian_dm = array();
                    $harian_dm = $data_harian->dm_harian_hari($tarikh, $dm->pdt_bil);
                    if(!empty($harian_dm)){
                    $highest_bn = 0;
                    ?>
                    <tr>
                        ...
                        <td>
                            <?php echo $harian_dm->hddt_keluar_mengundi; ?>%</td>
                        <td><?php 
                        ...
                        foreach($senarai_calon as $calon): 
                            $grading = $data_grading->hari_pdm_dun($calon->pencalonan_bil, $dm->pdt_bil, $tarikh);
                            $grading_hari_ini = number_format(0,2,".",",");
                            if(!empty($grading)){
                                $grading_hari_ini = number_format($grading->sgpdt_peratus,2,".",",");
                            }
                            $parti = $data_parti->parti($calon->pencalonan_parti);
                            ?>
                            <td style="<?php echo $parti->parti_warna; ?>">
                                <?php echo $grading_hari_ini; $seratus = $seratus + $grading_hari_ini; ?>%      
                            </td>
                            <td style="<?php echo $parti->parti_warna; ?>"><?php $pengundi = ($grading_hari_ini/100)*$jumlah_jangkaan_keluar_mengundi; 
                        ...
                        <td style="<?php if($seratus >= 100.01){ echo "background-color:red; color:white;"; } ?><?php if($seratus <= 99.99){ echo "background-color:yellow; color:black;"; } ?>"><?php echo $seratus; ?>%</td>
                ...
                <tfoot>
                    <tr class="bg-light">
                        ...
                        <td style="<?php echo $status_grading_dun['warna']; ?>"><?php echo $status_grading_dun['grade']; ?></td>
                    </tr>
                    <tr>
                        <th>JUSTIFIKASI</th>
