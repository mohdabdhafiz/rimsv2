...
<?php foreach($senaraiProgram as $program): 
    ...
    $senaraiSemakLaporan = array(
      array(
        "val" => "A",
        "tajuk" => "BAHAGIAN A - MAKLUMAT AM PROGRAM",
        "selesai" => FALSE
      ),
      array(
        "val" => "B",
        "tajuk" => "BAHAGIAN B - TAJUK HEBAHAN / CERAMAH PROGRAM",
        "selesai" => FALSE
      ),
      array(
        "val" => "C",
        "tajuk" => "BAHAGIAN C - PENGISIAN PROGRAM",
        "selesai" => FALSE
      ),
      array(
        "val" => "D",
        "tajuk" => "BAHAGIAN D - LOKASI PROGRAM",
        "selesai" => FALSE
      ),
      array(
        "val" => "E",
        "tajuk" => "BAHAGIAN E - PENGLIBATAN KOMUNITI",
        "selesai" => FALSE
      ),
      array(
        "val" => "F",
        "tajuk" => "BAHAGIAN F - SENARAI OBP",
        "selesai" => FALSE
      ),
      array(
        "val" => "G",
        "tajuk" => "BAHAGIAN G - PENGLIBATAN KELAB MALAYSIAKU",
        "selesai" => FALSE
      ),
      array(
        "val" => "H",
        "tajuk" => "BAHAGIAN H - KERJASAMA AGENSI LAIN",
        "selesai" => FALSE
      ),
      array(
        "val" => "I",
        "tajuk" => "BAHAGIAN I - EDARAN PENERBITAN",
        "selesai" => FALSE
      ),
      array(
        "val" => "J",
        "tajuk" => "BAHAGIAN J - LAPORAN GAMBAR",
        "selesai" => FALSE
      ),
      array(
        "val" => "K",
        "tajuk" => "BAHAGIAN K - PAUTAN MEDIA SOSIAL / LIPUTAN MEDIA",
        "selesai" => TRUE
      ),
      array(
        "val" => "L",
        "tajuk" => "BAHAGIAN L - LAPORAN KERATAN AKHBAR",
        "selesai" => TRUE
      )
    );
    $bilanganSemakan = count($senaraiSemakLaporan);
    $bilanganSenaraiSelesai = count($senaraiSemakLaporan);
    for($i = 0; $i < $bilanganSemakan; $i++){
      foreach($semakanLaporan as $sl){
        if($sl->ps_tajuk == $senaraiSemakLaporan[$i]['tajuk'] && $sl->ps_isi){
          $senaraiSemakLaporan[$i]['selesai'] = TRUE;
        }
      }
    }
  ?>

<?php if(strpos($st, 'PENGESAHAN HANTAR PPD') !== FALSE && !empty($akaunPpd)): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      Pengesahan Pegawai Penerangan Daerah</h1>
      <p>
        <strong>Status Laporan Semasa:</strong>
        <br><?= strtoupper($program->program_status) ?>
      </p>
      <p>
        <strong>Pengesahan:</strong>
        <br>Program ini telah dilaksanakan seperti ketetapan tempat, tarikh dan masa.
      </p>
      <p><strong>Senarai Pengisian Laporan:</strong></p>
        <div class="row g-3">
          <?php for($i = 0; $i < $bilanganSemakan; $i++): ?>
          <div class="col-12 col-lg-3 col-md-4 col-sm-6 text-center small">
            <div class="d-flex justify-content-start align-items-start">
              <?php if($senaraiSemakLaporan[$i]['selesai']): ?>
            <h3 class="text-success"><i class="bi bi-check2"></i></h3>
            <?php else: ?>
            <h3 class="text-danger"><i class="bi bi-exclamation-lg"></i></h3>
              <?php 
              $bilanganSenaraiSelesai--;
              endif; ?>
            <span class="text-start ms-3"><a href="#<?= strtolower($senaraiSemakLaporan[$i]['val']) ?>"><?= $senaraiSemakLaporan[$i]['tajuk'] ?></a></span>
            </div>
          </div>
          <?php endfor; ?>
        </div>
      <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahLaksanaNegeri">
      <i class="bi bi-reply"></i>
        Sah</button>
    </div>
  </div>

  <div class="modal fade" id="sahLaksanaNegeri" tabindex="-1">
...
<?php if(strpos($st, 'PENGESAHAN PERANCANGAN PPD') !== FALSE && !empty($akaunPpd)): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      ...
      <p>
        <strong>Pengesahan:</strong>
        <br>Program ini akan dilaksanakan seperti ketetapan tempat, tarikh dan masa.
      </p>
      <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahPpd">
      <i class="bi bi-reply"></i>
        Sah</button>
    </div>
  </div>

  <div class="modal fade" id="sahPpd" tabindex="-1">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
                    ...
                    <div class="modal-footer">
                      <button type="button" class="btn btn-outline-secondary shadow-sm" data-bs-dismiss="modal">
                        <i class="bi bi-x-octagon"></i>
                        Tutup</button>
                      <?= form_open('program/sahRancangPPBPKPM') ?>
                        <input type="hidden" name="inputProgramBil" value="<?= $program->program_bil ?>">
                        <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
                        <button type="submit" class="btn btn-outline-primary shadow-sm">
                        <i class="bi bi-reply"></i>
...
<?php if(strpos($st, 'DRAF') !== FALSE): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      Penghantaran Pengesahan Pegawai Penerangan Daerah</h1>
      <p>
        <strong>Status Laporan Semasa:</strong>
        <br><?= strtoupper($program->program_status) ?>
      </p>
      <p>
        <strong>Penghantaran Untuk Pengesahan Pegawai Penerangan Daerah:</strong>
        <br>Saya telah membuat semakan dan saya ingin menghantar laporan ini untuk pengesahan Pegawai Penerangan Daerah untuk dijadikan perancangan Pejabat Penerangan Daerah.</p>
        
        

        <button type="button" class="btn btn-outline-primary shadow-sm mt-3"  data-bs-toggle="modal" data-bs-target="#sahLaporan">
      <i class="bi bi-reply"></i>
        Hantar</button>
  ...
  <div class="modal fade" id="sahLaporan" tabindex="-1">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
...
<?php endif; ?>
  ...
  <?php 
    if(strpos($st, 'JADUAL AKTIVITI') !== FALSE && $pengguna->bil == $program->program_pelapor && $program->program_tarikh_masa < date("Y-m-d H:i:s")): ?>
  <div class="card">
    <div class="card-body">
      <div class="card-title">
        <i class="bi bi-receipt"></i>
        Penghantaran Laporan</div>
      <p>Saya sudah membuat pengemaskinian maklumat program untuk menghantar laporan ini kepada Pegawai Penerangan Daerah.</p>
      <p><strong>Senarai Pengisian Laporan:</strong></p>
        <div class="row g-3">
          <?php for($i = 0; $i < $bilanganSemakan; $i++): ?>
          <div class="col-12 col-lg-3 col-md-4 col-sm-6 text-center small">
      ...
      <button <?php if($bilanganSemakan != $bilanganSenaraiSelesai){ echo "disabled"; } ?> type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#hantarLaporan">
      <i class="bi bi-reply"></i>
        Hantar</button>
    </div>
  </div>
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                        <i class="bi bi-x-octagon"></i>
                        Tutup</button>
                      <?= form_open('program/sahLaksanaPPD') ?>
                        <input type="hidden" name="inputProgramBil" value="<?= $program->program_bil ?>">
                        <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
                        <button type="submit" class="btn btn-outline-primary shadow-sm">
                        <i class="bi bi-reply"></i>
  ...
  <?php 
    $st = strtoupper($program->program_status);
    if(strpos($st, 'JADUAL AKTIVITI') !== FALSE): ?>
  <div class="card">
    <div class="card-body">
      <div class="card-title text-danger">
        <i class="bi bi-x-circle"></i>
        Pembatalan Laporan</div>
      <p>Saya sudah berbincang bersama penyelia saya untuk membuat pembatalan penghantaran laporan ini kepada Bahagian Perkhidmatan Komunikasi (BPKPM), Jabatan Penerangan Malaysia (JaPen).</p>
  ...
  $bolehEdit = FALSE;
  $bolehEdit = strpos($st, 'JADUAL AKTIVITI') !== FALSE && $pengguna->bil == $program->program_pelapor && $program->program_tarikh_masa < date("Y-m-d H:i:s") || 
  strpos($st, 'DRAF') !== FALSE && $pengguna->bil == $program->program_pelapor || 
  (strpos($st, 'PENGESAHAN PERANCANGAN PPD') !== FALSE && !empty($akaunPpd)) ||
  strpos($st, 'PELAKSANAAN') !== FALSE ||
  (strpos($st, 'PENGESAHAN HANTAR PPD') !== FALSE && !empty($akaunPpd));


  //DELETE CONDITION
  $bolehPadam = FALSE;
  $bolehPadam = strpos($st, 'DRAF') !== FALSE;
  ?>
  ...
  <div class="card" id='a'>
    <div class="card-body">
      ...
      <div class="row g-3">
      <p class="col-12">
        <span class="small"><strong>Status Laporan:</strong></span> <span class="text-danger"><strong><?= strtoupper($program->program_status) ?></strong></span>
      </p>
      <p class="col-12 col-lg-3">
        <span class="small"><strong>Nama Pelapor:</strong></span>
        <br><?= strtoupper($program->nama_penuh) ?>, <?= strtoupper($program->program_no_telefon) ?>
      </p>
      <p class="col-12 col-lg-3">
                <span class="small"><strong>Nama Program:</strong></span><br>
                <?php echo strtoupper($program->jt_nama); ?>
            </p>
            <p class="col-12 col-lg-3"><span class="small"><strong>Tarikh Program:</strong></span><br>
            <?php echo date_format(date_create($program->program_tarikh_masa), "d.m.Y"); ?></p>
            <p class="col-12 col-lg-3"><span class="small"><strong>Masa Program:</strong></span><br> 
            ...
            <p class="col-12 col-lg-3"><span class="small"><strong>Negeri:</strong></span>
              <br><?= strtoupper($program->nt_nama) ?>
            </p>
            <p class="col-12 col-lg-3"><span class="small"><strong>Daerah:</strong></span>
              <br><?= strtoupper($program->nama) ?>
            </p>
            <p class="col-12 col-lg-3"><span class="small"><strong>Parlimen:</strong></span>
              <br><?= strtoupper($program->pt_nama) ?>
            </p>
            <p class="col-12 col-lg-3">
              <span class="small"><strong>DUN:</strong></span>
              <br><?= strtoupper($program->dun_nama) ?>
            </p>
            <p class="col-12 col-lg-3">
              <span class="small"><strong>Perasmi:</strong></span>
              <br><?= strtoupper($program->program_perasmi) ?>
            </p>
            <p class="col-12 col-lg-3"><span class="small"><strong>Bilangan Khalayak:</strong></span>
...
<div class="form-control mb-3">
  ...
  <div class="form-check">
    ...
    <label class="form-check-label" for="inputJenisProgram<?= $jenisProgram->jt_bil ?>">
      <?= strtoupper($jenisProgram->jt_nama) ?>
    </label>
  </div>
  <?php endforeach; ?>
</div>
...
<?php if(!empty($senaraiNegeri)): ?>
        ...
        <select name="inputNegeri" id="inputNegeri" class="form-control" required>
          <option value="" <?php if($program->program_negeri == ""){ echo 'selected'; } ?>>Sila pilih..</option>
            <?php foreach($senaraiNegeri as $negeri): ?>
                <option value="<?= $negeri->nt_bil ?>" <?php if($negeri->nt_bil == $program->program_negeri){ echo 'selected'; } ?>><?= strtoupper($negeri->nt_nama) ?></option>
            <?php endforeach; ?>
        </select>
        ...
        <select name="inputDaerah" id="inputDaerah" class="form-control" required>
          <option value="" <?php if($program->program_daerah == ""){ echo 'selected'; } ?>>Sila pilih..</option>
            <?php foreach($senaraiDaerah as $daerah): 
                ...
                <option value="<?= $daerah->bil ?>" <?php if($daerah->bil == $program->program_daerah){ echo 'selected'; } ?>><?= strtoupper($daerah->nama) ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="form-floating mb-3">
        <select name="inputParlimen" id="inputParlimen" class="form-control">
            ...
            <?php foreach($senaraiParlimen as $parlimen): ?>
                <option value="<?= $parlimen->pt_bil ?>" <?php if($parlimen->pt_bil == $program->program_parlimen){ echo 'selected'; } ?> ><?= strtoupper($parlimen->pt_nama) ?></option>
            <?php endforeach; ?>
        </select>
        ...
        <div class="form-floating mb-3">
            <select name="inputDun" id="inputDun" class="form-control">
                ...
                <?php foreach($senaraiDun as $dun): ?>
                    <option value="<?= $dun->dun_bil ?>" <?php if($dun->dun_bil == $program->program_dun){ echo 'selected'; } ?>><?= strtoupper($dun->dun_nama) ?></option>
                <?php endforeach; ?>
            </select>
  ...
  <div class="alert alert-warning mb-3">
    ...
    NOTA:</strong> Sila pastikan untuk masukkan <strong>NAMA PENUH</strong> perasmi dan <strong>JAWATAN</strong> perasmi.
  </div>
...
<div class="card" id="b">
  <div class="card-body">
    ...
    <h1 class="card-title">
      <i class="bi bi-card-heading"></i>
      Bahagian B - Naratif / Tajuk Hebahan / Ceramah Program
    </h1>
    <?php if($bolehEdit): ?>
    ...
    <ol>
      ...
      <li><?= strtoupper($tajuk->kandungan_program_kandungan) ?></li>
      <?php endforeach; ?>
    </ol>
    <?php endif; ?>
...
<div class="modal fade" id="tambahKandungan" tabindex="-1">
                <div class="modal-dialog modal-xl">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
                        <i class="bi bi-gear"></i>
                        Kemaskini Naratif / Tajuk Hebahan / Ceramah Program
                      </h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <table class="table table-bordered table-sm">
                          ...
                          <?php foreach($senaraiKandungan as $kandungan): ?>
                              <tr>
                                <td><?= strtoupper($kandungan->senarai_kandungan_kandungan) ?></td>
                                <td>
                              ...
                              <?php endforeach; ?>
                              <tr>
                                <th>Tajuk Hebahan / Ceramah Program Tambahan</th>
                                <th>Tindakan</th>
                              </tr>
                              <tr>
                                <?= form_open('program/tambahKandunganProgram') ?>
                                <input type="hidden" name="inputProgram" value="<?= $program->program_bil ?>">
                                  ...
                                  <td>
                                    <div class="form-floating">
                                      <input type="text" name="inputKandungan" id="inputKandungan" class="form-control" placeholder="Lain-Lain:" required>
                                      <label for="inputKandungan" class="form-label">Lain-Lain:</label>
                                    </div>
                                  </td>
                                  <td>
                                    <button type="submit" class="btn btn-sm btn-outline-primary shadow-sm">
                            ...
                            <div class="small text-muted">
                              <span class="text-danger">PENTING:</span> Sila pastikan program-program yang dirancang / dilaksanakan mengikut senarai naratif yang telah diselia oleh urus setia.
                            </div>
                      <?php if(!empty($senaraiTajuk)): ?>
                      <hr>
                      <h5>Senarai Naratif Tajuk Hebahan / Ceramah Program</h5>
                      <ol>
                        <?php foreach($senaraiTajuk as $tajuk): ?>
                        <li>
                          <?= form_open('program/padamKandunganProgram') ?>
                              ...
                              <input type="hidden" name="inputStatus" value="<?= $program->program_status ?>">
                              <div class="d-flex justify-content-between align-items-center">
                                <span><?= strtoupper($tajuk->kandungan_program_kandungan) ?></span>
  ...
  <div class="card-body">
    ...
    <ol>
      ...
      <li><?= strtoupper($tajuk->pengisian_program_pengisian) ?></li>
      <?php endforeach; ?>
    </ol>
    <?php endif; ?>
                    ...
                    <div class="modal-body">
                      ...
                      <table class="table table-bordered table-sm mt-2">
                          ...
                          <?php foreach($senaraiPengisianProgram as $pengisian): ?>
                              <tr>
                                <td><?= strtoupper($pengisian->senarai_pengisian_pengisian) ?></td>
                                <td>
                              ...
                              <tr>
                                <td>
                                  ...
                                  <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
                                  <div class="form-floating">
                                    <input type="text" name="inputpengisian" id="inputpengisian" class="form-control" placeholder="Lain-Lain:" required>
                                    <label for="inputpengisian" class="form-label">Lain-Lain:</label>
                                  </div>
                                </td>
                                <td>
                                  <button type="submit" class="btn btn-sm btn-outline-primary shadow-sm">
                      ...
                      <ol>
                        ...
                        <li>
                          <?= form_open('program/padampengisianProgram') ?>
                              ...
                              <input type="hidden" name="inputStatus" value="<?= $program->program_status ?>">
                              <div class="d-flex justify-content-between align-items-center">
                                <span><?= strtoupper($tajuk->pengisian_program_pengisian) ?></span>
                                <button type="submit" class="btn btn-outline-danger shadow-sm btn-sm">
                                  <i class="bi bi-trash"></i>
                                </button>
                              </div>
    ...
    <?php if(count($senaraiLokasi) > 0): ?>
      <ol>
        <?php foreach($senaraiLokasi as $lokasi): ?>
          <li><?= strtoupper($lokasi->lokasi_program_lokasi) ?></li>
        <?php endforeach; ?>
      </ol>
      <?php endif; ?>
      <?php if(count($senaraiLokasi) <= 0): ?>
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      <div class="alert alert-warning mb-3">
                        <strong>
                        <i class="bi bi-cone-striped"></i>  
                        NOTA:</strong> Sila pastikan untuk masukkan <strong>ALAMAT PENUH</strong> lokasi.
                      </div>
                      <div class="form-floating mb-3">
                    ...
                    <div class="table-responsive">
                      <table class="table table-sm table-bordered table-striped">
                        ...
                        <tbody>
                          <?php foreach($senaraiLokasi as $lokasi): ?>
                          <tr>
                            <td><?= strtoupper($lokasi->lokasi_program_lokasi) ?></td>
...
<div class="card" id="e">
  <div class="card-body">
    ...
    <?php if(count($senaraiKomuniti) <= 0): 
      ...
      <?php if($senaraiSemakLaporan['4']['tajuk'] == "BAHAGIAN E - PENGLIBATAN KOMUNITI" && !$senaraiSemakLaporan['4']['selesai']): ?>
      <?= form_open("program/sahTiada") ?>
        <div class="alert alert-info">
          <h1 class="alert-heading">Pengesahan</h1>
          <div class="d-flex justify-content-start align-items-center mb-3">
            <input type="checkbox" name="inputSah" id="inputSah">
            <span class="ms-3">Tiada penglibatan mana-mana komuniti dalam program ini.</span>
          </div>
          <input type="hidden" name="inputProgram" value="<?= $program->program_bil ?>">
          <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
          <input type="hidden" name="inputTajuk" value="BAHAGIAN E - PENGLIBATAN KOMUNITI">
          <input type="hidden" name="inputSegmen" value="e">
          <button type="submit" class="btn btn-outline-primary shadow-sm">
                ...
                <div class="modal-dialog modal-xl">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      <p>Bilangan Komuniti : <?= count($senaraiPilihanKomuniti) ?> buah komuniti</p>
                      <div class="table-responsive">
                        <table class="table table-sm table-bordered">
                          <thead>
                            <tr>
                            ...
                            <tr>
                              ...
                              foreach($senaraiKomuniti as $ak){
                                if($ak->komuniti_program_komuniti == $komuniti->komuniti_bil){
                                  $ada = TRUE;
                                  $bk[$komuniti->komuniti_bil] = $ak->komuniti_program_bilangan_kehadiran;
                                }
                              }
                              ?>
                              ...
                              <td><?= strtoupper($komuniti->komuniti_nama) ?></td>
...
<div class="card" id="f">
    ...
    <?php if(!empty($senaraiObp)): ?>
      <ol>
        ...
        <li>
          <?= strtoupper($o->obp_program_nama) ?>
          <br> <?= strtoupper($o->obp_program_jawatan) ?>
        </li>
        <?php endforeach; ?>
      </ol>
    <?php endif; ?>
    <?php if(empty($senaraiObp)): ?>
      ...
      <?php if($senaraiSemakLaporan['5']['tajuk'] == "BAHAGIAN F - SENARAI OBP" && !$senaraiSemakLaporan['5']['selesai']): ?>
        <?= form_open("program/sahTiada") ?>
        <div class="alert alert-info">
          <h1 class="alert-heading">Pengesahan</h1>
          <div class="d-flex justify-content-start align-items-center mb-3">
            <input type="checkbox" name="inputSah" id="inputSah">
            <span class="ms-3">Tiada penglibatan OBP dalam program ini.</span>
          </div>
          <input type="hidden" name="inputProgram" value="<?= $program->program_bil ?>">
          <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
          <input type="hidden" name="inputTajuk" value="BAHAGIAN F - SENARAI OBP">
          <input type="hidden" name="inputSegmen" value="f">
          <button type="submit" class="btn btn-outline-primary shadow-sm">
            <i class="bi bi-check2"></i>
            Sah
          </button>
                        ...
                        <table class="table table-hover table-bordered table-striped">
                          ...
                          <tbody>
                            <?php $count = 0; foreach($senaraiPeranan as $peranan):
                               ...
                               $listObp = $dataObp->obpProgram2($peranan->peranan_bil, $senaraiDaerah);
                               if(!empty($listObp)):
                                foreach($listObp as $o):
                                  $ada = FALSE;
                                  foreach($senaraiObp as $obpJoin){
                                    if($obpJoin->obp_program_obp == $o->obp_id && $obpJoin->obp_program_nama == $o->obp_nama){
                                      $ada = TRUE;
                                    }
                                  }
                               ...
                               ?>
                            <tr>
                              <td><input type="checkbox" name="inputObp[]" id="inputObpBil_<?= $count++ ?>" value="<?= $o->obp_id ?>_<?= $o->peranan_id ?>" <?php if($ada){ echo "checked"; } ?>></td>
                              <td><?= strtoupper($o->obp_nama) ?></td>
                              <td><?= strtoupper($o->obp_jawatan) ?></td>
                            </tr>
                            <?php 
                                endforeach;
                              endif;
    ...
    <div class="row g-3 mb-3">
      <div class="col-12 col-lg-3 col-md-6 col-sm-12">
        <p>
          ...
          <br><?= strtoupper($senaraiMaklumatTambahanKelabMalaysiaku->mtpk_peringkat) ?>
        </p>
        <p>
          <strong>Mod Pelaksanaan</strong>
          <br><?= strtoupper($senaraiMaklumatTambahanKelabMalaysiaku->mtpk_mod_pelaksanaan) ?>
        </p>
      </div>
      <div class="col-12 col-lg-3 col-md-6 col-sm-12">
        <p>
    ...
    <h5>Senarai Penglibatan Kelab Malaysiaku</h5>
      <ol>
        <?php  foreach($senaraiKelab as $kelab): ?>
          <li><a href="<?= site_url('kelabmalaysiaku/bil/'.$kelab->kelabmalaysiaku_bil) ?>"><?= strtoupper($kelab->kelabmalaysiaku_nama) ?></a></li>
        <?php endforeach; ?>
      </ol>
    <?php endif; ?>
    <?php if(empty($senaraiKelab)): ?>
      ...
      <?php if($senaraiSemakLaporan['6']['tajuk'] == "BAHAGIAN G - PENGLIBATAN KELAB MALAYSIAKU" && !$senaraiSemakLaporan['6']['selesai']): ?>
      <?= form_open("program/sahTiada") ?>
        <div class="alert alert-info">
          <h1 class="alert-heading">Pengesahan</h1>
          <div class="d-flex justify-content-start align-items-center mb-3">
            <input type="checkbox" name="inputSah" id="inputSah">
            <span class="ms-3">Tiada penglibatan Kelab Malaysiaku dalam program ini.</span>
          </div>
          <input type="hidden" name="inputProgram" value="<?= $program->program_bil ?>">
          <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
          <input type="hidden" name="inputTajuk" value="BAHAGIAN G - PENGLIBATAN KELAB MALAYSIAKU">
          <input type="hidden" name="inputSegmen" value="g">
          <button type="submit" class="btn btn-outline-primary shadow-sm">
            <i class="bi bi-check2"></i>
            Sah
          </button>
                        ...
                        <table class="table table-bordered">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td><?= strtoupper($kelab->kelabmalaysiaku_nama) ?></td>
                            </tr>
                            <?php endforeach; ?>
                          </tbody>
                        </table>
...
<div class="card" id="h">
  <div class="card-body">
    ...
    <ol>
      ...
      <li><?= strtoupper($tajuk->agensi_program_agensi) ?></li>
      <?php endforeach; ?>
    </ol>
    <?php endif; ?>
    <?php if(count($senaraiAgensi) <= 0): ?>
      ...
      <?php if($senaraiSemakLaporan['7']['tajuk'] == "BAHAGIAN H - KERJASAMA AGENSI LAIN" && !$senaraiSemakLaporan['7']['selesai']): ?>
      <?= form_open("program/sahTiada") ?>
        <div class="alert alert-info">
          <h1 class="alert-heading">Pengesahan</h1>
          <div class="d-flex justify-content-start align-items-center mb-3">
            <input type="checkbox" name="inputSah" id="inputSah">
            <span class="ms-3">Tiada penglibatan agensi lain dalam program ini.</span>
          </div>
          <input type="hidden" name="inputProgram" value="<?= $program->program_bil ?>">
          <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
          <input type="hidden" name="inputTajuk" value="BAHAGIAN H - KERJASAMA AGENSI LAIN">
          <input type="hidden" name="inputSegmen" value="h">
          <button type="submit" class="btn btn-outline-primary shadow-sm">
            <i class="bi bi-check2"></i>
            Sah
          </button>
                    ...
                    <div class="modal-body">
                      ...
                      <table class="table table-bordered table-sm mt-2">
                          ...
                          <?php foreach($senaraiAgensiProgram as $agensi): ?>
                              <tr>
                                <td><?= strtoupper($agensi->senarai_agensi_agensi) ?></td>
                                <td>
                        ...
                        <div class="row g-3">
                          <div class="col-12 col-lg-11">
                            <div class="form-floating">
                              <input type="text" name="inputagensi" id="inputagensi" class="form-control" placeholder="Lain-Lain:" required>
                              <label for="inputagensi" class="form-label">Lain-Lain:</label>
                            </div>
                          </div>
                      ...
                      <ol>
                        ...
                        <li>
                          <?= form_open('program/padamAgensiProgram') ?>
                              ...
                              <input type="hidden" name="inputStatus" value="<?= $program->program_status ?>">
                              <div class="d-flex justify-content-between align-items-center">
                                <span><?= strtoupper($tajuk->agensi_program_agensi) ?></span>
                                <button type="submit" class="btn btn-outline-danger shadow-sm btn-sm">
                                  <i class="bi bi-trash"></i>
                                </button>
                              </div>
    ...
    <div class="table-responsive">
      <table class="table table-sm table-bordered">
        ...
        <tbody>
          ...
          <tr>
            <td><?= strtoupper($tajuk->penerbitan_program_penerbitan) ?></td>
                    ...
                    <div class="modal-body">
                      ...
                      <table class="table table-bordered table-sm mt-2">
                          ...
                          <?php foreach($senaraiPenerbitanProgram as $penerbitan): ?>
                              <tr>
                                <td><?= strtoupper($penerbitan->senarai_penerbitan_penerbitan) ?></td>
...
<div class="card" id='j'>
  <div class="card-body">
    ...
    Nota:
    <ol>
        <li>Diingatkan bahawa tuan/puan perlu memuatnaik <strong>lima (5) keping gambar</strong> dan <strong>satu (1)</strong> paparan video (Video hendaklah berdurasi kurang daripada 2 minit dan rakaman hendaklah berkait dengan pengisian program).</li>
      </ol>
...
?>
    ...
    <div class="row g-3">
      ...
      <div class="col-12 col-lg-4">
        ...
        <?php endif; ?>
        <p class="mt-1"><?= strtoupper($gambar->gambar_program_deskripsi) ?></p>
        </div>
      </div>
      <?php 
          }
                ...
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    ...
                    <?php echo form_open_multipart('foto/prosesTambahGambarProgram');?>
                      <input type="file" name="input_userfile" class="form-control mb-3" accept="image/*, video/*" required/>
                      <div class="form-floating mb-3">
                      ...
                      <div class="row g-3">
                        ...
                        <div class="col-12 col-lg-6 col-md-4 col-sm-12">
                          <div class="text-center">
                            ...
                            <p class="small text-muted"><?= strtoupper($g->gambar_program_deskripsi) ?></p>
                            <?php endif; ?>
  ...
  <div class="card-body">
    ...
    <p>
      ...
      <span class="small text-muted"><?= count($senaraiPautan) ?></span>
                      ...
                      <?php if(count($senaraiPautan) <= 0): ?>
      <div class="alert alert-warning">
        TIADA PAUTAN MEDIA SOSIAL / LIPUTAN MEDIA.
      </div>
    <?php endif; ?>                  
...
<div class="card" id='l'>
  <div class="card-body">
    ...
    <?php if(empty($senaraiKeratanAkhbar)): ?>
      ...
      <?php if($senaraiSemakLaporan['11']['tajuk'] == "BAHAGIAN L - LAPORAN KERATAN AKHBAR" && !$senaraiSemakLaporan['11']['selesai']): ?>
      <?= form_open("program/sahTiada") ?>
        <div class="alert alert-info">
          <h1 class="alert-heading">Pengesahan</h1>
          <div class="d-flex justify-content-start align-items-center mb-3">
            <input type="checkbox" name="inputSah" id="inputSah">
            <span class="ms-3">Masih tiada keratan akhbar untuk program ini.</span>
          </div>
          <input type="hidden" name="inputProgram" value="<?= $program->program_bil ?>">
          <input type="hidden" name="inputPengguna" value="<?= $pengguna->bil ?>">
          <input type="hidden" name="inputTajuk" value="BAHAGIAN L - LAPORAN KERATAN AKHBAR">
          <input type="hidden" name="inputSegmen" value="l">
          <button type="submit" class="btn btn-outline-primary shadow-sm">
            <i class="bi bi-check2"></i>
            Sah
          </button>
    ...
    <div class="row g-3">
      ...
      <div class="col-12 col-lg-4">
        ...
        <?php endif; ?>
        <p class="mt-1"><?= strtoupper($keratanAkhbar->keratan_akhbar_program_deskripsi) ?></p>
        </div>
      </div>
      <?php 
          }
                      ...
                      <div class="row g-3">
                        ...
                        <div class="col-12 col-lg-6 col-md-4 col-sm-12">
                          <div class="text-center">
                            ...
                            <p class="small text-muted"><?= strtoupper($g->keratan_akhbar_program_deskripsi) ?></p>
                            <?php endif; ?>
  ...
  <div class="card-body">
    ...
    <?php if(!empty($statusLaporan)): ?>
      <div class="alert alert-success border-success">
        <strong>Status Semasa : </strong><span class="text-danger"><strong><?= strtoupper($statusLaporan->status_program_status) ?></strong></span> - <?= strtoupper($statusLaporan->status_program_deskripsi) ?>
      </div>
    <?php endif; ?>
      ...
      <table class="table table-bordered table-hover table-striped">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?= strtoupper($stats->status_program_status) ?></td>
            <td><?= strtoupper($stats->status_program_deskripsi) ?></td>
            <td><?= strtoupper($stats->nama_penuh) ?></td>
