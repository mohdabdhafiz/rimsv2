...
<div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Harian PARLIMEN <?= $parlimen->pt_nama ?></li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
...
<div class="">
    ...
    <div class="card">
        <div class="card-body">
            <h1 class="card-title">Grading Harian Parlimen</h1>
            <div class="row g-1">
                <?php foreach($senaraiParlimenPilihanraya as $parlimenPru): ?>
                <div class="col-auto">
                    <?= form_open('grading/parlimenPilihanraya') ?>
                        <input type="hidden" name="inputParlimenBil" value="<?= $parlimenPru->pt_bil ?>">
                        <input type="hidden" name="inputPilihanrayaBil" value="<?= $harian->pruBil ?>">
                        <button type="submit" class="btn btn-outline-primary shadow-sm"><?= $parlimenPru->pt_nama ?></button>
                    </form>
                </div>
                <?php endforeach; ?>
            </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-title">Grading Harian bagi <?= $parlimen->pt_nama ?> untuk <?= $pilihanraya->pilihanraya_singkatan ?></h5>
            ...
            <div class="table-responsive">
                <table class="table table-borderless">
                    ...
                    <tr>
                        <th class="text-secondary">Parlimen</th>
                    ...
                    <tr>
                        ...
                        <td style="<?= warnaGrading($harian->harian_parlimen_grading) ?>"><?= $harian->harian_parlimen_grading ?></td>
                    </tr>
                    <tr>
                        <th class="text-secondary">Peratus Keluar Mengundi (%)</th>
                        <td><?= $harian->harian_parlimen_keluar_mengundi ?></td>
                    </tr>
                    <tr>
                        <th class="text-secondary">Pengundi Atas Pagar / Rosak (%)</th>
                        <td><?= $harian->harian_parlimen_atas_pagar ?></td>
                    </tr>
                    <tr>
                        <th class="text-secondary">Kemaskini</th>
                        <td><?= $harian->harian_parlimen_pengguna_waktu ?></td>
                    </tr>
                </table>
            </div>
                ...
                <table class="table table-bordered table-hover">
                    <thead>
                        <tr>
                            ...
                            foreach($senaraiCalon as $calon): 
                                $jumlahPengundiCalon[$calon->pencalonan_parlimen_bil] = 0; ?>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= $dm->ppt_nama ?></td>
                            <td>
                                <?= number_format($dm->ppt_bilangan_pengundi, 0, '', ',') ?>
                                <?php $jumlahBilanganPengundi = $jumlahBilanganPengundi + $dm->ppt_bilangan_pengundi; ?>
                            </td>
                            <td><?= $dm->hdpt_keluar_mengundi ?>%</td>
                            <?php $pengundiKeluar = 0;
                            $pengundiKeluar = ($dm->hdpt_keluar_mengundi/100) * $dm->ppt_bilangan_pengundi;
                            $jumlahPengundiKeluar = $jumlahPengundiKeluar + $pengundiKeluar;
                            ?>
                            <td><?= number_format(floor($pengundiKeluar), 0, '', ',') ?></td>
                            <?php 
                            $pengundiDM = $dm->hdpt_keluar_mengundi/100 * $dm->ppt_bilangan_pengundi;
                            $highest = 0;
                            $calonMenangBil = 0;
                            $tempPutihHighest = 0;
                            $tempNotPutihHighest = 0;
                            foreach($senaraiCalon as $calon):
                                $grading = $dataGrading->hari($harian->tarikh, $calon->pencalonan_parlimen_bil, $dm->ppt_bil);
                                    $peratusGrading = 0;
                                    if(!empty($grading->sgppt_peratus)){
                                        $peratusGrading = $grading->sgppt_peratus;
                                    }
                            $pengundi = $peratusGrading/100 * floor($pengundiDM);
                            $jumlahPengundiCalon[$calon->pencalonan_parlimen_bil] = $jumlahPengundiCalon[$calon->pencalonan_parlimen_bil] + $pengundi;
                            ?>
                                ...
                                <td style="<?= $calon->parti_warna ?>">
                                    ...
                                    $parti_pilihan = $dataParti->pilihan_parti($calon->pencalonan_parlimen_partiBil); 
                            ...
                            <td>
                                <?= $dm->hdpt_atas_pagar ?>%
                                <?php 
                                $jumlahAtasPagar = $jumlahAtasPagar + $dm->hdpt_atas_pagar;
                                $seratus = $seratus + $dm->hdpt_atas_pagar; 
                                $warnaSeratus = "";
                                if($seratus < 100){
                                    $warnaSeratus = 'background:yellow; color:black';
                                }elseif($seratus > 100){
                                    $warnaSeratus = 'background:red; color:white';
                                }
                                ?>
                            </td>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <?php
                            $parlimenKeluarMengundi = ($jumlahPengundiKeluar / $jumlahBilanganPengundi) * 100;
                            ?>
                            <th><?= number_format($parlimenKeluarMengundi, 2, '.', '') ?>%</th>
                            ...
                            $warnaParti = "";
                            foreach($senaraiCalon as $calon): ?>
                            <th style="<?= $calon->parti_warna ?>"><?= number_format($jumlahPengundiCalon[$calon->pencalonan_parlimen_bil], 0, '', ',') ?></th>
                            <?php 
                            $peratusPengundiCalon = ($jumlahPengundiCalon[$calon->pencalonan_parlimen_bil] / (($parlimenKeluarMengundi/100) * $jumlahBilanganPengundi)) * 100;
                            $jumlahSeratus = $jumlahSeratus + $peratusPengundiCalon;
                            ?>
                            <th style="<?= $calon->parti_warna ?>"><?= number_format($peratusPengundiCalon, 2, '.', '') ?>%</th>
                            <?php 
                            $parti_pilihan = $dataParti->pilihan_parti($calon->pencalonan_parlimen_partiBil); 
                            $pengundi = floor($jumlahPengundiCalon[$calon->pencalonan_parlimen_bil]);
                            if(!empty($parti_pilihan)){
                                if($pengundi >= $tempPutihHighest){
                                    $tempPutihHighest = $pengundi;
                                }
                            ...
                            <th>
                                <?php 
                                $peratusMajoriti = 0;
                                $peratusMajoriti = ($majoriti/(($parlimenKeluarMengundi/100) * $jumlahBilanganPengundi)) * 100;
                                echo number_format($peratusMajoriti, 2, '.', ','); 
                                ?>%
                            </th>
                            <?php 
    ...
    <div class="card">
        <div class="card-body">
            ...
            <p><?= $harian->harian_parlimen_ulasan ?></p>
        </div>
    </div>
