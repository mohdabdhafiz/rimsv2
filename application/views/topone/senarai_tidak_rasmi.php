<div class="row g-3 mb-3">
    ...
    <?php foreach($senarai_rekod as $rekod): 
        $parlimen = $data_parlimen->parlimen_bil($rekod->rppt_parlimen_bil); 
        $calon_baru = $data_calon->calon($rekod->rppt_calon_baru);
        $calon_sebelum = $data_calon->calon($rekod->rppt_calon_sebelum);
        $calon_selepas = $data_calon->calon($rekod->rppt_calon_selepas); 
        ?>
            ...
            <div class="row g-3 mb-3">
                <div class="col-12 col-lg-4 d-flex justify-content-center align-items-start">
                    <div class="w-100 d-flex align-items-center">
                        <img src="<?= base_url() ?>assets/bendera/<?= $data_negeri->nama_foto($parlimen->pt_negeri)->nt_nama_fail ?>" alt="<?= strtoupper($parlimen->pt_negeri) ?>" class="img-fluid p-1" style="object-fit: contain;max-width: 1000px;height: 40px;">
                        <h2 class='p-1'>PARLIMEN <?= strtoupper($parlimen->pt_nama) ?></h2> 
                    </div>
                </div>
                <div class="col-12 col-lg-4 d-flex justify-content-center align-items-start">
                    <div class="w-100">
                    <?php if($rekod->rppt_kategori_perubahan == "TUKAR"){ 
                        $parti_sebelum = $data_parti->parti($calon_sebelum->pencalonan_parlimen_partiBil); 
                        $foto_sebelum = $data_foto->foto($parti_sebelum->parti_logo);    
                        $parti_selepas = $data_parti->parti($calon_selepas->pencalonan_parlimen_partiBil); 
                    ...
                    <?php if($rekod->rppt_kategori_perubahan == "BARU"){ 
                        $parti_baru = $data_parti->parti($calon_baru->pencalonan_parlimen_partiBil); 
                    ...
                    <?php if($rekod->rppt_kategori_perubahan == '10000') { ?>
                    <p class="text-center"><span class="display-4">MAJORITI > 10,000</span></p>
                    <?php } ?>

                    <?php if($rekod->rppt_kategori_perubahan == 'PENGEMASKINIAN MAKLUMAT') { ?>
                ...
                <?php
                $ls = $data_calon->jangkaan($parlimen->pt_bil, $calon_baru->pencalonan_parlimen_pilihanrayaBil);
                if(!empty($ls)){ 
                ...
                <div class="col-12 col-lg-12">
                    ...
                    <tr>
                        ...
                        <tr>
                            ...
                            <th class="text-center" valign="middle" colspan=<?= $colspan ?>><?= $rekod->rppt_peti_undi ?></th>
                        </tr>
                    </table>
                </div>
            </div>
            <p class="small text-muted text-end">Kemaskini terakhir pada <?= $rekod->rppt_waktu ?></p>
        </div>
    </div>
    <?php endforeach; ?>
