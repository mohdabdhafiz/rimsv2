    ...
    <div class="card">
        <div class="card-body">
            ...
            <?php echo form_open('ketuaUnit/prosesLantikan'); ?>
                <div class="form-floating mb-3">
                    <select name="inputKetuaUnit" id="inputKetuaUnit" class="form-control" required>
                        ...
                        <?php foreach($senaraiAnggota as $anggota): ?>
                            <option 
                            <?php
                            if(!empty($ketuaUnit->bil)){ 
                                if($ketuaUnit->bil == $anggota->bil){ 
                                    echo "selected"; 
                                } 
                            }
                            ?> 
                            value="<?= $anggota->bil ?>"><?= $anggota->nama_penuh ?>, <?= $anggota->pekerjaan ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <input type="hidden" name="inputGelaranJawatan" value="Penolong Pengarah BPKPM">
