...
foreach ($senaraiStatus as $stats) {
    $statusNama = strtoupper($stats->status_program_status);
    $statusMasa = strtotime($stats->status_program_pengguna_waktu); // Convert time to timestamp

    $status = [
        "susunan" => 99, // Default high value to catch undefined statuses
        "statusNama" => $statusNama,
        "statusPengguna" => strtoupper($stats->nama_penuh),
        "statusMasa" => $statusMasa, // Store as timestamp for easy comparison
        "statusMasaPaparan" => $stats->status_program_pengguna_waktu
    ];
    ...
    switch ($statusNama) {
        case "DRAF":
            $status["susunan"] = 0;
            break;
        case "PENGESAHAN PERANCANGAN PPD":
            $status["susunan"] = 1;
            break;
        case "JADUAL AKTIVITI":
            $status["susunan"] = 2;
            break;
        case "PENGESAHAN HANTAR PPD":
            $status["susunan"] = 3;
            break;
        case "PENGESAHAN HANTAR NEGERI":
            $status["susunan"] = 4;
            break;
        case "SELESAI":
        case "HANTAR":
    ...
    // Store only the latest status entry based on timestamp
    if (!isset($susunanStatus[$statusNama]) || $statusMasa > $susunanStatus[$statusNama]["statusMasa"]) {
        $susunanStatus[$statusNama] = $status;
    }
}

// Convert associative array to indexed array for sorting
$susunanStatus = array_values($susunanStatus);

// Sorting the array based on 'susunan'
array_multisort(array_column($susunanStatus, "susunan"), SORT_ASC, $susunanStatus);

$tarikhProgram = $program->program_tarikh_masa;
?>
    ...
    <div class="table-responsive">
      <table class="table table-bordered">
        <tr>
          ...
          <?php foreach($susunanStatus as $stats): ?>
          <th
          <?php if(strtoupper($stats["statusNama"]) == "JADUAL AKTIVITI"): ?>
            class="bg-warning"
          <?php endif; ?>
          <?php if(strtoupper($stats["statusNama"]) == "SELESAI" || strtoupper($stats["statusNama"]) == "HANTAR"): ?>
            class="bg-success text-white"
          <?php endif; ?>
          <?php if(strtoupper($stats["statusNama"]) == "KUIRI"): ?>
        ...
        <tr>
          ...
          <th
          <?php if(strtoupper($stats["statusNama"]) == "JADUAL AKTIVITI"): 
            $tarikhJadualAktiviti = $stats["statusMasa"];
            ?>
            class="bg-warning"
          <?php endif; ?>
          <?php if(strtoupper($stats["statusNama"]) == "SELESAI" || strtoupper($stats["statusNama"]) == "HANTAR"): ?>
...
<?php if(strpos($st, "SELESAI") !== FALSE && $pengguna->pengguna_status !== ""): ?>
<div class="card">
  <div class="card-body">
    <h1 class="card-title">
      <i class="bi bi-arrow-left-right"></i>
      Kuiri Urus Setia Bahagian Perkhidmatan Komunikasi dan Pembangunan Masyarakat
    </h1>
    <p>Saya ingin memulangkan semula laporan ini kepada urus setia negeri untuk tindakan lanjutan.</p>
    <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahKuiri">
      <i class="bi bi-reply"></i>
        Kuiri</button>
  </div>
</div>

<div class="modal fade" id="sahKuiri" tabindex="-1">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
                        <i class="bi bi-receipt"></i>
                        Penghantaran Kuiri Laporan</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <?= form_open('program/kuiriLaporan') ?>
                        <div class="mb-3">
                          <label for="inputJustifikasi" class="form-label">Justifikasi Kuiri Laporan</label>
...
<?php if(strpos($st, 'PENGESAHAN HANTAR PP BPKPM') !== FALSE && $pengguna->bil == $akaunKu->bil): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      Pengesahan Pelaksanaan Penolong Pengarah, Bahagian Perkhidmatan Komunikasi dan Pembangunan Masyarakat</h1>
      <p>
      ...
      <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahSelesai">
      <i class="bi bi-reply"></i>
        Sah</button>
    </div>
  </div>

  <div class="modal fade" id="sahSelesai" tabindex="-1">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
...
<?php if(strpos($st, 'PENGESAHAN HANTAR BPKPM') !== FALSE): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      Pengesahan Pelaksanaan Bahagian Perkhidmatan Komunikasi dan Pembangunan Masyarakat</h1>
      <p>
        <strong>Status Laporan Semasa:</strong>
        <br><?= strtoupper($program->program_status) ?>
      </p>
      ...
      <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahHantarPp">
      <i class="bi bi-reply"></i>
        Sah</button>
    </div>
  </div>

  <div class="modal fade" id="sahHantarPp" tabindex="-1">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
...
<?php if(strpos($st, 'PENGESAHAN PERANCANGAN BPKPM') !== FALSE): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      Pengesahan Perancangan Bahagian Perkhidmatan Komunikasi dan Pembangunan Masyarakat</h1>
      <p>
        <strong>Status Laporan Semasa:</strong>
        <br><?= strtoupper($program->program_status) ?>
      </p>
      ...
      <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahPerancanganBpkpm">
      <i class="bi bi-reply"></i>
        Sah</button>
    </div>
  </div>

  <div class="modal fade" id="sahPerancanganBpkpm" tabindex="-1">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
...
<?php if(strpos($st, 'PENGESAHAN PERANCANGAN PP BPKPM') !== FALSE && $pengguna->bil == $akaunKu->bil): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      Pengesahan Penolong Pengarah</h1>
      <p>
        <strong>Status Laporan Semasa:</strong>
        <br><?= $program->program_status ?>
      </p>
      <p>
        <strong>Pengesahan:</strong>
        <br>Program ini akan dilaksanakan seperti ketetapan tempat, tarikh dan masa.
      </p>
      <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahRancangPPBPKPM">
      <i class="bi bi-reply"></i>
        Sah</button>
    </div>
  </div>

  <div class="modal fade" id="sahRancangPPBPKPM" tabindex="-1">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">
...
<?php if(strpos($st, 'DRAF BPKPM') !== FALSE): ?>
  <div class="card">
    <div class="card-body">
      <h1 class="card-title">
      <i class="bi bi-receipt"></i>
      ...
      <p>
        ...
        <button type="button" class="btn btn-outline-primary shadow-sm"  data-bs-toggle="modal" data-bs-target="#sahLaporan">
      <i class="bi bi-reply"></i>
        Hantar</button>
    </div>
  </div>
...
<?php endif; ?>
  ...
  <?php 
    if(strpos($st, 'JADUAL AKTIVITI') !== FALSE && $pengguna->bil == $program->program_pelapor): ?>
  <div class="card">
    <div class="card-body">
      <div class="card-title">
        <i class="bi bi-receipt"></i>
  ...
  <div class="card">
    <div class="card-body">
      ...
      <p>Saya sudah berbincang dengan pelapor supaya laporan ini dibatalkan. Penyelia pelapor turut dimaklumkan.</p>
  ...
  $bolehEdit = FALSE;
  $bolehEdit = strpos($st, 'JADUAL AKTIVITI') !== FALSE || 
  strpos($st, 'DRAF BPKPM') !== FALSE || 
  (strpos($st, 'PENGESAHAN PERANCANGAN PP BPKPM') !== FALSE && $pengguna->bil == $akaunKu->bil) ||
  strpos($st, 'PELAKSANAAN') !== FALSE ||
  strpos($st, 'PENGESAHAN PERANCANGAN BPKPM') !== FALSE ||
  strpos($st, 'PENGESAHAN HANTAR BPKPM') !== FALSE ||
  (strpos($st, 'PENGESAHAN HANTAR PP BPKPM') !== FALSE && $pengguna->bil == $akaunKu->bil);
  ...
  <div class="card" id='a'>
    <div class="card-body">
      ...
      <p class="col-12 col-lg-4 col-md-3 col-sm-6">
        <span class="small"><strong>Nama Pelapor:</strong></span>
        <br><?= strtoupper($program->nama_penuh) ?>, <?= $program->program_no_telefon ?>
      </p>
      <p class="col-12 col-lg-4 col-md-3 col-sm-6">
                <span class="small"><strong>Nama Program:</strong></span><br>
                <?php echo strtoupper($program->jt_nama); ?>
            </p>
            <p class="col-12 col-lg-4 col-md-3 col-sm-6"><span class="small"><strong>Tarikh Program:</strong></span><br>
            <?php echo date_format(date_create($program->program_tarikh_masa), "Y-m-d"); ?></p>
            <p class="col-12 col-lg-4 col-md-3 col-sm-6"><span class="small"><strong>Masa Program:</strong></span><br> 
            <?php echo date_format(date_create($program->program_tarikh_masa), "H:i:s"); ?></p>
            <p class="col-12 col-lg-4 col-md-3 col-sm-6"><span class="small"><strong>Negeri:</strong></span>
              <br><?= strtoupper($program->nt_nama) ?>
            </p>
            <p class="col-12 col-lg-4 col-md-3 col-sm-6"><span class="small"><strong>Daerah:</strong></span>
              <br><?= strtoupper($program->nama) ?>
            </p>
            <p class="col-12 col-lg-4 col-md-3 col-sm-6"><span class="small"><strong>Parlimen:</strong></span>
            ...
            <p class="col-12 col-lg-4 col-md-3 col-sm-6"><span class="small"><strong>Bilangan Khalayak:</strong></span>
...
<?php if($pengguna->pengguna_status == ""): ?>
<div class="form-floating mb-3">
  <select name="inputPelapor" id="inputPelapor" class="form-control" required>
    ...
    <option value="<?= $pelapor->bil ?>" <?php if($program->program_pelapor == $pelapor->bil){ echo "selected"; } ?>><?= strtoupper($pelapor->nama_penuh) ?></option>
    <?php endforeach; ?>
  </select>
...
<?php if($pengguna->pengguna_status !== ""): ?>
<div class="form-floating mb-3">
  <input type="text" id="tempPelapor" disabled value="<?= strtoupper($pengguna->nama_penuh) ?>" class="form-control">
  <label for="inputPelapor" class="form-label">Nama Pegawai Pelapor: <span style="color:red;">*</span></label>
        ...
        <select name="inputDaerah" id="inputDaerah" class="form-control" required>
          <option value="" <?php if($program->program_daerah == ""){ echo 'selected'; } ?>>Sila pilih..</option>
            <?php foreach($senaraiDaerah as $daerah): 
                ...
                <option value="<?= $daerah->bil ?>" <?php if($daerah->bil == $program->program_daerah){ echo 'selected'; } ?>><?= strtoupper($daerah->nt_nama) ?> - <?= strtoupper($daerah->nama) ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="form-floating mb-3">
        <select name="inputParlimen" id="inputParlimen" class="form-control">
            ...
            <?php foreach($senaraiParlimen as $parlimen): ?>
                <option value="<?= $parlimen->pt_bil ?>" <?php if($parlimen->pt_bil == $program->program_parlimen){ echo 'selected'; } ?> ><?= strtoupper($parlimen->pt_negeri) ?> - <?= strtoupper($parlimen->pt_nama) ?></option>
            <?php endforeach; ?>
        </select>
        ...
        <div class="form-floating mb-3">
            <select name="inputDun" id="inputDun" class="form-control">
                ...
                <?php foreach($senaraiDun as $dun): ?>
                    <option value="<?= $dun->dun_bil ?>" <?php if($dun->dun_bil == $program->program_dun){ echo 'selected'; } ?>><?= strtoupper($dun->dun_negeri) ?> - <?= strtoupper($dun->dun_nama) ?></option>
                <?php endforeach; ?>
            </select>
